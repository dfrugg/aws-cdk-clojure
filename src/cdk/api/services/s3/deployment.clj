...
(defn bucket-deployment-builder
  "The bucket-deployment-builder function buildes out new instances of 
BucketDeployment$Builder using the provided configuration.  Each field is set as follows:
...
| `websiteRedirectLocation` | java.lang.String | [[cdk.support/lookup-entry]] | `:website-redirect-location` |"
  [stack id config]
  (let [builder (BucketDeployment$Builder/create stack id)]
...
(defn bucket-deployment-props-builder
  "The bucket-deployment-props-builder function buildes out new instances of 
BucketDeploymentProps$Builder using the provided configuration.  Each field is set as follows:
...
| `websiteRedirectLocation` | java.lang.String | [[cdk.support/lookup-entry]] | `:website-redirect-location` |"
  [stack id config]
  (let [builder (BucketDeploymentProps$Builder.)]
    (when-let [data (bucket-access-control config id :access-control)]
      (. builder accessControl data))
    (when-let [data (lookup-entry config id :cache-control)]
      (. builder cacheControl data))
...
(defn deploy-time-substituted-file-builder
  "The deploy-time-substituted-file-builder function buildes out new instances of 
DeployTimeSubstitutedFile$Builder using the provided configuration.  Each field is set as follows:
...
| `substitutions` | java.util.Map | [[cdk.support/lookup-entry]] | `:substitutions` |"
  [stack id config]
  (let [builder (DeployTimeSubstitutedFile$Builder/create stack id)]
    (when-let [data (lookup-entry config id :destination-bucket)]
      (. builder destinationBucket data))
...
(defn deploy-time-substituted-file-props-builder
  "The deploy-time-substituted-file-props-builder function buildes out new instances of 
DeployTimeSubstitutedFileProps$Builder using the provided configuration.  Each field is set as follows:
...
| `substitutions` | java.util.Map | [[cdk.support/lookup-entry]] | `:substitutions` |"
  [stack id config]
  (let [builder (DeployTimeSubstitutedFileProps$Builder.)]
    (when-let [data (lookup-entry config id :destination-bucket)]
      (. builder destinationBucket data))
    (when-let [data (lookup-entry config id :destination-key)]
      (. builder destinationKey data))
...
(defn deployment-source-context-builder
  "The deployment-source-context-builder function buildes out new instances of 
DeploymentSourceContext$Builder using the provided configuration.  Each field is set as follows:

| Field | DataType | Lookup Function | Data Key |
|---|---|---|---|
| `handlerRole` | software.amazon.awscdk.services.iam.IRole | [[cdk.support/lookup-entry]] | `:handler-role` |"
  [stack id config]
  (let [builder (DeploymentSourceContext$Builder.)]
