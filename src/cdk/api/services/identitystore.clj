...
(defn cfn-group-membership-builder
  "Creates a  `CfnGroupMembership$Builder` instance using a scope and ID, applies the data configuration using the [[cfn-group-membership-builder>]] function, then builds it.  Takes the following arguments: 
| Argument | DataType | Description |
|---|---|---|
| `scope` | software.constructs.Construct | The parent scope construct of the object being built. |
| `id` | String or Keyword or Symbol | Value to use as both the ID of the object being build and the namespace when looking up configuration values. |
...
(defn cfn-group-membership-member-id-property-builder
  "Creates a  `CfnGroupMembership$MemberIdProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-group-membership-member-id-property-builder>]] function, then builds it.  Takes the following arguments: 
...
(defn cfn-group-membership-props-builder
  "Creates a  `CfnGroupMembershipProps$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-group-membership-props-builder>]] function, then builds it.  Takes the following arguments: 
