...
(defn action-artifact-bounds-builder>
  "The action-artifact-bounds-builder> function updates a ActionArtifactBounds$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :max-inputs)]
    (. builder maxInputs data))
  (when-some [data (lookup-entry config id :max-outputs)]
    (. builder maxOutputs data))
  (when-some [data (lookup-entry config id :min-inputs)]
    (. builder minInputs data))
  (when-some [data (lookup-entry config id :min-outputs)]
    (. builder minOutputs data))
  (.build builder))


(defn action-artifact-bounds-builder
  "Creates a  `ActionArtifactBounds$Builder` instance using a no-argument constructor, applies the data configuration using the [[action-artifact-bounds-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (action-artifact-bounds-builder> (new ActionArtifactBounds$Builder) id config))


(defn action-bind-options-builder>
  "The action-bind-options-builder> function updates a ActionBindOptions$Builder instance using the provided configuration.
...
(defn action-bind-options-builder
  "Creates a  `ActionBindOptions$Builder` instance using a no-argument constructor, applies the data configuration using the [[action-bind-options-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (action-bind-options-builder> (new ActionBindOptions$Builder) id config))


(defn action-config-builder>
  "The action-config-builder> function updates a ActionConfig$Builder instance using the provided configuration.
...
(defn action-config-builder
  "Creates a  `ActionConfig$Builder` instance using a no-argument constructor, applies the data configuration using the [[action-config-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (action-config-builder> (new ActionConfig$Builder) id config))


(defn action-properties-builder>
  "The action-properties-builder> function updates a ActionProperties$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :artifact-bounds)]
    (. builder artifactBounds data))
  (when-some [data (action-category config id :category)]
...
(defn action-properties-builder
  "Creates a  `ActionProperties$Builder` instance using a no-argument constructor, applies the data configuration using the [[action-properties-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (action-properties-builder> (new ActionProperties$Builder) id config))


(defn cfn-custom-action-type-artifact-details-property-builder>
  "The cfn-custom-action-type-artifact-details-property-builder> function updates a CfnCustomActionType$ArtifactDetailsProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :maximum-count)]
    (. builder maximumCount data))
  (when-some [data (lookup-entry config id :minimum-count)]
    (. builder minimumCount data))
  (.build builder))


(defn cfn-custom-action-type-artifact-details-property-builder
  "Creates a  `CfnCustomActionType$ArtifactDetailsProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-custom-action-type-artifact-details-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-custom-action-type-artifact-details-property-builder> (new CfnCustomActionType$ArtifactDetailsProperty$Builder) id config))


(defn cfn-custom-action-type-builder>
  "The cfn-custom-action-type-builder> function updates a CfnCustomActionType$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :input-artifact-details)]
    (. builder inputArtifactDetails data))
  (when-some [data (lookup-entry config id :output-artifact-details)]
...
(defn cfn-custom-action-type-builder
  "Creates a  `CfnCustomActionType$Builder` instance using a scope and ID, applies the data configuration using the [[cfn-custom-action-type-builder>]] function, then builds it.  Takes the following arguments: 
| Argument | DataType | Description |
|---|---|---|
...
| `config` | map | Data configuration |"
  ...
  (cfn-custom-action-type-builder> (CfnCustomActionType$Builder/create scope (name id)) id config))


(defn cfn-custom-action-type-configuration-properties-property-builder>
  "The cfn-custom-action-type-configuration-properties-property-builder> function updates a CfnCustomActionType$ConfigurationPropertiesProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :name)]
    (. builder name data))
  (when-some [data (lookup-entry config id :queryable)]
...
(defn cfn-custom-action-type-configuration-properties-property-builder
  "Creates a  `CfnCustomActionType$ConfigurationPropertiesProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-custom-action-type-configuration-properties-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-custom-action-type-configuration-properties-property-builder> (new CfnCustomActionType$ConfigurationPropertiesProperty$Builder) id config))


(defn cfn-custom-action-type-props-builder>
  "The cfn-custom-action-type-props-builder> function updates a CfnCustomActionTypeProps$Builder instance using the provided configuration.
...
(defn cfn-custom-action-type-props-builder
  "Creates a  `CfnCustomActionTypeProps$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-custom-action-type-props-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-custom-action-type-props-builder> (new CfnCustomActionTypeProps$Builder) id config))


(defn cfn-custom-action-type-settings-property-builder>
  "The cfn-custom-action-type-settings-property-builder> function updates a CfnCustomActionType$SettingsProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :entity-url-template)]
    (. builder entityUrlTemplate data))
  (when-some [data (lookup-entry config id :execution-url-template)]
    (. builder executionUrlTemplate data))
  (when-some [data (lookup-entry config id :revision-url-template)]
    (. builder revisionUrlTemplate data))
  (when-some [data (lookup-entry config id :third-party-configuration-url)]
    (. builder thirdPartyConfigurationUrl data))
  (.build builder))


(defn cfn-custom-action-type-settings-property-builder
  "Creates a  `CfnCustomActionType$SettingsProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-custom-action-type-settings-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-custom-action-type-settings-property-builder> (new CfnCustomActionType$SettingsProperty$Builder) id config))


(defn cfn-pipeline-action-declaration-property-builder>
  "The cfn-pipeline-action-declaration-property-builder> function updates a CfnPipeline$ActionDeclarationProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :action-type-id)]
    (. builder actionTypeId data))
  (when-some [data (lookup-entry config id :configuration)]
    (. builder configuration data))
  (when-some [data (lookup-entry config id :input-artifacts)]
    (. builder inputArtifacts data))
  (when-some [data (lookup-entry config id :name)]
    (. builder name data))
  (when-some [data (lookup-entry config id :namespace)]
    (. builder namespace data))
  (when-some [data (lookup-entry config id :output-artifacts)]
...
(defn cfn-pipeline-action-declaration-property-builder
  "Creates a  `CfnPipeline$ActionDeclarationProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-pipeline-action-declaration-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-pipeline-action-declaration-property-builder> (new CfnPipeline$ActionDeclarationProperty$Builder) id config))


(defn cfn-pipeline-action-type-id-property-builder>
  "The cfn-pipeline-action-type-id-property-builder> function updates a CfnPipeline$ActionTypeIdProperty$Builder instance using the provided configuration.
...
(defn cfn-pipeline-action-type-id-property-builder
  "Creates a  `CfnPipeline$ActionTypeIdProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-pipeline-action-type-id-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-pipeline-action-type-id-property-builder> (new CfnPipeline$ActionTypeIdProperty$Builder) id config))


(defn cfn-pipeline-artifact-store-map-property-builder>
  "The cfn-pipeline-artifact-store-map-property-builder> function updates a CfnPipeline$ArtifactStoreMapProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :artifact-store)]
...
(defn cfn-pipeline-artifact-store-map-property-builder
  "Creates a  `CfnPipeline$ArtifactStoreMapProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-pipeline-artifact-store-map-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-pipeline-artifact-store-map-property-builder> (new CfnPipeline$ArtifactStoreMapProperty$Builder) id config))


(defn cfn-pipeline-artifact-store-property-builder>
  "The cfn-pipeline-artifact-store-property-builder> function updates a CfnPipeline$ArtifactStoreProperty$Builder instance using the provided configuration.
...
| Field | DataType | Lookup Function | Data Key |
|---|---|---|---|
| `encryptionKey` | software.amazon.awscdk.services.codepipeline.CfnPipeline$EncryptionKeyProperty | [[cdk.support/lookup-entry]] | `:encryption-key` |
...
(defn cfn-pipeline-artifact-store-property-builder
  "Creates a  `CfnPipeline$ArtifactStoreProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-pipeline-artifact-store-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-pipeline-artifact-store-property-builder> (new CfnPipeline$ArtifactStoreProperty$Builder) id config))


(defn cfn-pipeline-blocker-declaration-property-builder>
  "The cfn-pipeline-blocker-declaration-property-builder> function updates a CfnPipeline$BlockerDeclarationProperty$Builder instance using the provided configuration.
...
(defn cfn-pipeline-blocker-declaration-property-builder
  "Creates a  `CfnPipeline$BlockerDeclarationProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-pipeline-blocker-declaration-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-pipeline-blocker-declaration-property-builder> (new CfnPipeline$BlockerDeclarationProperty$Builder) id config))
...
"
  ...
  (when-some [data (lookup-entry config id :artifact-store)]
    (. builder artifactStore data))
  (when-some [data (lookup-entry config id :artifact-stores)]
    (. builder artifactStores data))
  (when-some [data (lookup-entry config id :disable-inbound-stage-transitions)]
    (. builder disableInboundStageTransitions data))
  (when-some [data (lookup-entry config id :execution-mode)]
    (. builder executionMode data))
  (when-some [data (lookup-entry config id :name)]
    (. builder name data))
  (when-some [data (lookup-entry config id :pipeline-type)]
    (. builder pipelineType data))
  (when-some [data (lookup-entry config id :restart-execution-on-update)]
...
(defn cfn-pipeline-encryption-key-property-builder>
  "The cfn-pipeline-encryption-key-property-builder> function updates a CfnPipeline$EncryptionKeyProperty$Builder instance using the provided configuration.
...
(defn cfn-pipeline-encryption-key-property-builder
  "Creates a  `CfnPipeline$EncryptionKeyProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-pipeline-encryption-key-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-pipeline-encryption-key-property-builder> (new CfnPipeline$EncryptionKeyProperty$Builder) id config))


(defn cfn-pipeline-failure-conditions-property-builder>
  "The cfn-pipeline-failure-conditions-property-builder> function updates a CfnPipeline$FailureConditionsProperty$Builder instance using the provided configuration.
...
(defn cfn-pipeline-failure-conditions-property-builder
  "Creates a  `CfnPipeline$FailureConditionsProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-pipeline-failure-conditions-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-pipeline-failure-conditions-property-builder> (new CfnPipeline$FailureConditionsProperty$Builder) id config))


(defn cfn-pipeline-git-branch-filter-criteria-property-builder>
  "The cfn-pipeline-git-branch-filter-criteria-property-builder> function updates a CfnPipeline$GitBranchFilterCriteriaProperty$Builder instance using the provided configuration.
...
(defn cfn-pipeline-git-branch-filter-criteria-property-builder
  "Creates a  `CfnPipeline$GitBranchFilterCriteriaProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-pipeline-git-branch-filter-criteria-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-pipeline-git-branch-filter-criteria-property-builder> (new CfnPipeline$GitBranchFilterCriteriaProperty$Builder) id config))


(defn cfn-pipeline-git-configuration-property-builder>
  "The cfn-pipeline-git-configuration-property-builder> function updates a CfnPipeline$GitConfigurationProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :pull-request)]
    (. builder pullRequest data))
  (when-some [data (lookup-entry config id :push)]
    (. builder push data))
  (when-some [data (lookup-entry config id :source-action-name)]
    (. builder sourceActionName data))
  (.build builder))


(defn cfn-pipeline-git-configuration-property-builder
  "Creates a  `CfnPipeline$GitConfigurationProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-pipeline-git-configuration-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-pipeline-git-configuration-property-builder> (new CfnPipeline$GitConfigurationProperty$Builder) id config))


(defn cfn-pipeline-git-file-path-filter-criteria-property-builder>
  "The cfn-pipeline-git-file-path-filter-criteria-property-builder> function updates a CfnPipeline$GitFilePathFilterCriteriaProperty$Builder instance using the provided configuration.
...
(defn cfn-pipeline-git-file-path-filter-criteria-property-builder
  "Creates a  `CfnPipeline$GitFilePathFilterCriteriaProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-pipeline-git-file-path-filter-criteria-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-pipeline-git-file-path-filter-criteria-property-builder> (new CfnPipeline$GitFilePathFilterCriteriaProperty$Builder) id config))


(defn cfn-pipeline-git-pull-request-filter-property-builder>
  "The cfn-pipeline-git-pull-request-filter-property-builder> function updates a CfnPipeline$GitPullRequestFilterProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :events)]
    (. builder events data))
  (when-some [data (lookup-entry config id :file-paths)]
    (. builder filePaths data))
  (.build builder))


(defn cfn-pipeline-git-pull-request-filter-property-builder
  "Creates a  `CfnPipeline$GitPullRequestFilterProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-pipeline-git-pull-request-filter-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-pipeline-git-pull-request-filter-property-builder> (new CfnPipeline$GitPullRequestFilterProperty$Builder) id config))


(defn cfn-pipeline-git-push-filter-property-builder>
  "The cfn-pipeline-git-push-filter-property-builder> function updates a CfnPipeline$GitPushFilterProperty$Builder instance using the provided configuration.
...
(defn cfn-pipeline-git-push-filter-property-builder
  "Creates a  `CfnPipeline$GitPushFilterProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-pipeline-git-push-filter-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-pipeline-git-push-filter-property-builder> (new CfnPipeline$GitPushFilterProperty$Builder) id config))


(defn cfn-pipeline-git-tag-filter-criteria-property-builder>
  "The cfn-pipeline-git-tag-filter-criteria-property-builder> function updates a CfnPipeline$GitTagFilterCriteriaProperty$Builder instance using the provided configuration.
...
(defn cfn-pipeline-git-tag-filter-criteria-property-builder
  "Creates a  `CfnPipeline$GitTagFilterCriteriaProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-pipeline-git-tag-filter-criteria-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-pipeline-git-tag-filter-criteria-property-builder> (new CfnPipeline$GitTagFilterCriteriaProperty$Builder) id config))


(defn cfn-pipeline-input-artifact-property-builder>
  "The cfn-pipeline-input-artifact-property-builder> function updates a CfnPipeline$InputArtifactProperty$Builder instance using the provided configuration.
...
(defn cfn-pipeline-input-artifact-property-builder
  "Creates a  `CfnPipeline$InputArtifactProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-pipeline-input-artifact-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-pipeline-input-artifact-property-builder> (new CfnPipeline$InputArtifactProperty$Builder) id config))


(defn cfn-pipeline-output-artifact-property-builder>
  "The cfn-pipeline-output-artifact-property-builder> function updates a CfnPipeline$OutputArtifactProperty$Builder instance using the provided configuration.
...
(defn cfn-pipeline-output-artifact-property-builder
  "Creates a  `CfnPipeline$OutputArtifactProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-pipeline-output-artifact-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-pipeline-output-artifact-property-builder> (new CfnPipeline$OutputArtifactProperty$Builder) id config))


(defn cfn-pipeline-pipeline-trigger-declaration-property-builder>
  "The cfn-pipeline-pipeline-trigger-declaration-property-builder> function updates a CfnPipeline$PipelineTriggerDeclarationProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :git-configuration)]
...
(defn cfn-pipeline-pipeline-trigger-declaration-property-builder
  "Creates a  `CfnPipeline$PipelineTriggerDeclarationProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-pipeline-pipeline-trigger-declaration-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-pipeline-pipeline-trigger-declaration-property-builder> (new CfnPipeline$PipelineTriggerDeclarationProperty$Builder) id config))
...
(defn cfn-pipeline-stage-declaration-property-builder>
  "The cfn-pipeline-stage-declaration-property-builder> function updates a CfnPipeline$StageDeclarationProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :blockers)]
...
(defn cfn-pipeline-stage-declaration-property-builder
  "Creates a  `CfnPipeline$StageDeclarationProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-pipeline-stage-declaration-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-pipeline-stage-declaration-property-builder> (new CfnPipeline$StageDeclarationProperty$Builder) id config))


(defn cfn-pipeline-stage-transition-property-builder>
  "The cfn-pipeline-stage-transition-property-builder> function updates a CfnPipeline$StageTransitionProperty$Builder instance using the provided configuration.
...
(defn cfn-pipeline-stage-transition-property-builder
  "Creates a  `CfnPipeline$StageTransitionProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-pipeline-stage-transition-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-pipeline-stage-transition-property-builder> (new CfnPipeline$StageTransitionProperty$Builder) id config))


(defn cfn-pipeline-variable-declaration-property-builder>
  "The cfn-pipeline-variable-declaration-property-builder> function updates a CfnPipeline$VariableDeclarationProperty$Builder instance using the provided configuration.
...
(defn cfn-pipeline-variable-declaration-property-builder
  "Creates a  `CfnPipeline$VariableDeclarationProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-pipeline-variable-declaration-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-pipeline-variable-declaration-property-builder> (new CfnPipeline$VariableDeclarationProperty$Builder) id config))


(defn cfn-webhook-builder>
  "The cfn-webhook-builder> function updates a CfnWebhook$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :name)]
    (. builder name data))
  (when-some [data (lookup-entry config id :register-with-third-party)]
    (. builder registerWithThirdParty data))
  (when-some [data (lookup-entry config id :target-action)]
    (. builder targetAction data))
  (when-some [data (lookup-entry config id :target-pipeline)]
    (. builder targetPipeline data))
  (when-some [data (lookup-entry config id :target-pipeline-version)]
    (. builder targetPipelineVersion data))
  (.build builder))


(defn cfn-webhook-builder
  "Creates a  `CfnWebhook$Builder` instance using a scope and ID, applies the data configuration using the [[cfn-webhook-builder>]] function, then builds it.  Takes the following arguments: 
| Argument | DataType | Description |
|---|---|---|
| `scope` | software.constructs.Construct | The parent scope construct of the object being built. |
| `id` | String or Keyword or Symbol | Value to use as both the ID of the object being build and the namespace when looking up configuration values. |
| `config` | map | Data configuration |"
  [^software.constructs.Construct scope id config]
  (cfn-webhook-builder> (CfnWebhook$Builder/create scope (name id)) id config))


(defn cfn-webhook-props-builder>
  "The cfn-webhook-props-builder> function updates a CfnWebhookProps$Builder instance using the provided configuration.
...
(defn cfn-webhook-props-builder
  "Creates a  `CfnWebhookProps$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-webhook-props-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-webhook-props-builder> (new CfnWebhookProps$Builder) id config))


(defn cfn-webhook-webhook-auth-configuration-property-builder>
  "The cfn-webhook-webhook-auth-configuration-property-builder> function updates a CfnWebhook$WebhookAuthConfigurationProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :allowed-ip-range)]
...
(defn cfn-webhook-webhook-auth-configuration-property-builder
  "Creates a  `CfnWebhook$WebhookAuthConfigurationProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-webhook-webhook-auth-configuration-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-webhook-webhook-auth-configuration-property-builder> (new CfnWebhook$WebhookAuthConfigurationProperty$Builder) id config))


(defn cfn-webhook-webhook-filter-rule-property-builder>
  "The cfn-webhook-webhook-filter-rule-property-builder> function updates a CfnWebhook$WebhookFilterRuleProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :match-equals)]
    (. builder matchEquals data))
  (.build builder))


(defn cfn-webhook-webhook-filter-rule-property-builder
  "Creates a  `CfnWebhook$WebhookFilterRuleProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-webhook-webhook-filter-rule-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-webhook-webhook-filter-rule-property-builder> (new CfnWebhook$WebhookFilterRuleProperty$Builder) id config))


(defn common-action-props-builder>
  "The common-action-props-builder> function updates a CommonActionProps$Builder instance using the provided configuration.
...
(defn common-action-props-builder
  "Creates a  `CommonActionProps$Builder` instance using a no-argument constructor, applies the data configuration using the [[common-action-props-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (common-action-props-builder> (new CommonActionProps$Builder) id config))


(defn common-aws-action-props-builder>
  "The common-aws-action-props-builder> function updates a CommonAwsActionProps$Builder instance using the provided configuration.
...
(defn common-aws-action-props-builder
  "Creates a  `CommonAwsActionProps$Builder` instance using a no-argument constructor, applies the data configuration using the [[common-aws-action-props-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (common-aws-action-props-builder> (new CommonAwsActionProps$Builder) id config))


(defn cross-region-support-builder>
  "The cross-region-support-builder> function updates a CrossRegionSupport$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :replication-bucket)]
...
(defn cross-region-support-builder
  "Creates a  `CrossRegionSupport$Builder` instance using a no-argument constructor, applies the data configuration using the [[cross-region-support-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cross-region-support-builder> (new CrossRegionSupport$Builder) id config))


(defn custom-action-property-builder>
  "The custom-action-property-builder> function updates a CustomActionProperty$Builder instance using the provided configuration.
...
(defn custom-action-property-builder
  "Creates a  `CustomActionProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[custom-action-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (custom-action-property-builder> (new CustomActionProperty$Builder) id config))


(defn custom-action-registration-builder>
  "The custom-action-registration-builder> function updates a CustomActionRegistration$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :action-properties)]
    (. builder actionProperties data))
  (when-some [data (lookup-entry config id :artifact-bounds)]
    (. builder artifactBounds data))
  (when-some [data (action-category config id :category)]
    (. builder category data))
  (when-some [data (lookup-entry config id :entity-url)]
    (. builder entityUrl data))
  (when-some [data (lookup-entry config id :execution-url)]
...
(defn custom-action-registration-builder
  "Creates a  `CustomActionRegistration$Builder` instance using a scope and ID, applies the data configuration using the [[custom-action-registration-builder>]] function, then builds it.  Takes the following arguments: 
| Argument | DataType | Description |
|---|---|---|
| `scope` | software.constructs.Construct | The parent scope construct of the object being built. |
| `id` | String or Keyword or Symbol | Value to use as both the ID of the object being build and the namespace when looking up configuration values. |
| `config` | map | Data configuration |"
  [^software.constructs.Construct scope id config]
  (custom-action-registration-builder> (CustomActionRegistration$Builder/create scope (name id)) id config))


(defn custom-action-registration-props-builder>
  "The custom-action-registration-props-builder> function updates a CustomActionRegistrationProps$Builder instance using the provided configuration.
...
(defn custom-action-registration-props-builder
  "Creates a  `CustomActionRegistrationProps$Builder` instance using a no-argument constructor, applies the data configuration using the [[custom-action-registration-props-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (custom-action-registration-props-builder> (new CustomActionRegistrationProps$Builder) id config))


(defn git-configuration-builder>
  "The git-configuration-builder> function updates a GitConfiguration$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :pull-request-filter)]
    (. builder pullRequestFilter data))
  (when-some [data (lookup-entry config id :push-filter)]
    (. builder pushFilter data))
  (when-some [data (lookup-entry config id :source-action)]
    (. builder sourceAction data))
  (.build builder))


(defn git-configuration-builder
  "Creates a  `GitConfiguration$Builder` instance using a no-argument constructor, applies the data configuration using the [[git-configuration-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (git-configuration-builder> (new GitConfiguration$Builder) id config))


(defn git-pull-request-filter-builder>
  "The git-pull-request-filter-builder> function updates a GitPullRequestFilter$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :branches-excludes)]
    (. builder branchesExcludes data))
  (when-some [data (lookup-entry config id :branches-includes)]
    (. builder branchesIncludes data))
  (when-some [data (lookup-entry config id :events)]
    (. builder events data))
  (when-some [data (lookup-entry config id :file-paths-excludes)]
    (. builder filePathsExcludes data))
  (when-some [data (lookup-entry config id :file-paths-includes)]
    (. builder filePathsIncludes data))
  (.build builder))


(defn git-pull-request-filter-builder
  "Creates a  `GitPullRequestFilter$Builder` instance using a no-argument constructor, applies the data configuration using the [[git-pull-request-filter-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (git-pull-request-filter-builder> (new GitPullRequestFilter$Builder) id config))


(defn git-push-filter-builder>
  "The git-push-filter-builder> function updates a GitPushFilter$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :tags-excludes)]
    (. builder tagsExcludes data))
  (when-some [data (lookup-entry config id :tags-includes)]
    (. builder tagsIncludes data))
  (.build builder))


(defn git-push-filter-builder
  "Creates a  `GitPushFilter$Builder` instance using a no-argument constructor, applies the data configuration using the [[git-push-filter-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (git-push-filter-builder> (new GitPushFilter$Builder) id config))


(defn pipeline-builder>
  "The pipeline-builder> function updates a Pipeline$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (execution-mode config id :execution-mode)]
    (. builder executionMode data))
  (when-some [data (lookup-entry config id :pipeline-name)]
    (. builder pipelineName data))
  (when-some [data (pipeline-type config id :pipeline-type)]
    (. builder pipelineType data))
  (when-some [data (lookup-entry config id :restart-execution-on-update)]
    (. builder restartExecutionOnUpdate data))
...
(defn pipeline-builder
  "Creates a  `Pipeline$Builder` instance using a scope and ID, applies the data configuration using the [[pipeline-builder>]] function, then builds it.  Takes the following arguments: 
| Argument | DataType | Description |
|---|---|---|
| `scope` | software.constructs.Construct | The parent scope construct of the object being built. |
| `id` | String or Keyword or Symbol | Value to use as both the ID of the object being build and the namespace when looking up configuration values. |
| `config` | map | Data configuration |"
  [^software.constructs.Construct scope id config]
  (pipeline-builder> (Pipeline$Builder/create scope (name id)) id config))


(defn pipeline-notify-on-options-builder>
  "The pipeline-notify-on-options-builder> function updates a PipelineNotifyOnOptions$Builder instance using the provided configuration.
...
(defn pipeline-notify-on-options-builder
  "Creates a  `PipelineNotifyOnOptions$Builder` instance using a no-argument constructor, applies the data configuration using the [[pipeline-notify-on-options-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (pipeline-notify-on-options-builder> (new PipelineNotifyOnOptions$Builder) id config))


(defn pipeline-props-builder>
  "The pipeline-props-builder> function updates a PipelineProps$Builder instance using the provided configuration.
...
(defn pipeline-props-builder
  "Creates a  `PipelineProps$Builder` instance using a no-argument constructor, applies the data configuration using the [[pipeline-props-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (pipeline-props-builder> (new PipelineProps$Builder) id config))
...
"
  ...
  (when-some [data (lookup-entry config id :stage-name)]
    (. builder stageName data))
  (when-some [data (lookup-entry config id :transition-disabled-reason)]
    (. builder transitionDisabledReason data))
  (when-some [data (lookup-entry config id :transition-to-enabled)]
...
(defn stage-placement-builder>
  "The stage-placement-builder> function updates a StagePlacement$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :just-after)]
    (. builder justAfter data))
  (when-some [data (lookup-entry config id :right-before)]
    (. builder rightBefore data))
  (.build builder))


(defn stage-placement-builder
  "Creates a  `StagePlacement$Builder` instance using a no-argument constructor, applies the data configuration using the [[stage-placement-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (stage-placement-builder> (new StagePlacement$Builder) id config))
...
"
  ...
  (when-some [data (lookup-entry config id :git-configuration)]
    (. builder gitConfiguration data))
  (when-some [data (provider-type config id :provider-type)]
    (. builder providerType data))
  (.build builder))


(defn trigger-builder
  "Creates a  `Trigger$Builder` instance using a no-argument create, applies the data configuration using the [[trigger-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (trigger-builder> (Trigger$Builder/create) id config))
...
(defn variable-builder>
  "The variable-builder> function updates a Variable$Builder instance using the provided configuration.
...
(defn variable-builder
  "Creates a  `Variable$Builder` instance using a no-argument create, applies the data configuration using the [[variable-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (variable-builder> (Variable$Builder/create) id config))


(defn variable-props-builder>
  "The variable-props-builder> function updates a VariableProps$Builder instance using the provided configuration.
...
(defn variable-props-builder
  "Creates a  `VariableProps$Builder` instance using a no-argument constructor, applies the data configuration using the [[variable-props-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (variable-props-builder> (new VariableProps$Builder) id config))