...
(defn nodejs-function-builder
  "The nodejs-function-builder function buildes out new instances of 
NodejsFunction$Builder using the provided configuration.  Each field is set as follows:
...
| `vpcSubnets` | software.amazon.awscdk.services.ec2.SubnetSelection | [[cdk.support/lookup-entry]] | `:vpc-subnets` |"
  [stack id config]
  (let [builder (NodejsFunction$Builder/create stack id)]
...
(defn nodejs-function-props-builder
  "The nodejs-function-props-builder function buildes out new instances of 
NodejsFunctionProps$Builder using the provided configuration.  Each field is set as follows:
...
| `vpcSubnets` | software.amazon.awscdk.services.ec2.SubnetSelection | [[cdk.support/lookup-entry]] | `:vpc-subnets` |"
  [stack id config]
  (let [builder (NodejsFunctionProps$Builder.)]
    (when-let [data (lookup-entry config id :adot-instrumentation)]
      (. builder adotInstrumentation data))
    (when-let [data (lookup-entry config id :allow-all-outbound)]
      (. builder allowAllOutbound data))
