...
"
  ...
  (when-some [data (lookup-entry config id :format)]
    (. builder format data))
  (when-some [data (lookup-entry config id :format-options)]
    (. builder formatOptions data))
  (when-some [data (lookup-entry config id :input)]
    (. builder input data))
  (when-some [data (lookup-entry config id :name)]
    (. builder name data))
  (when-some [data (lookup-entry config id :path-options)]
...
(defn cfn-dataset-csv-options-property-builder>
  "The cfn-dataset-csv-options-property-builder> function updates a CfnDataset$CsvOptionsProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :header-row)]
    (. builder headerRow data))
  (.build builder))


(defn cfn-dataset-csv-options-property-builder
  "Creates a  `CfnDataset$CsvOptionsProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-dataset-csv-options-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-dataset-csv-options-property-builder> (new CfnDataset$CsvOptionsProperty$Builder) id config))


(defn cfn-dataset-data-catalog-input-definition-property-builder>
  "The cfn-dataset-data-catalog-input-definition-property-builder> function updates a CfnDataset$DataCatalogInputDefinitionProperty$Builder instance using the provided configuration.
...
| `tempDirectory` | software.amazon.awscdk.services.databrew.CfnDataset$S3LocationProperty | [[cdk.support/lookup-entry]] | `:temp-directory` |
"
  ...
  (when-some [data (lookup-entry config id :temp-directory)]
    (. builder tempDirectory data))
  (.build builder))


(defn cfn-dataset-data-catalog-input-definition-property-builder
  "Creates a  `CfnDataset$DataCatalogInputDefinitionProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-dataset-data-catalog-input-definition-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-dataset-data-catalog-input-definition-property-builder> (new CfnDataset$DataCatalogInputDefinitionProperty$Builder) id config))


(defn cfn-dataset-database-input-definition-property-builder>
  "The cfn-dataset-database-input-definition-property-builder> function updates a CfnDataset$DatabaseInputDefinitionProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :database-table-name)]
    (. builder databaseTableName data))
  (when-some [data (lookup-entry config id :glue-connection-name)]
...
(defn cfn-dataset-database-input-definition-property-builder
  "Creates a  `CfnDataset$DatabaseInputDefinitionProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-dataset-database-input-definition-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-dataset-database-input-definition-property-builder> (new CfnDataset$DatabaseInputDefinitionProperty$Builder) id config))


(defn cfn-dataset-dataset-parameter-property-builder>
  "The cfn-dataset-dataset-parameter-property-builder> function updates a CfnDataset$DatasetParameterProperty$Builder instance using the provided configuration.
...
| Field | DataType | Lookup Function | Data Key |
|---|---|---|---|
| `createColumn` | java.lang.Boolean | [[cdk.support/lookup-entry]] | `:create-column` |
| `datetimeOptions` | software.amazon.awscdk.services.databrew.CfnDataset$DatetimeOptionsProperty | [[cdk.support/lookup-entry]] | `:datetime-options` |
...
"
  ...
  (when-some [data (lookup-entry config id :create-column)]
    (. builder createColumn data))
  (when-some [data (lookup-entry config id :datetime-options)]
...
(defn cfn-dataset-dataset-parameter-property-builder
  "Creates a  `CfnDataset$DatasetParameterProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-dataset-dataset-parameter-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-dataset-dataset-parameter-property-builder> (new CfnDataset$DatasetParameterProperty$Builder) id config))


(defn cfn-dataset-datetime-options-property-builder>
  "The cfn-dataset-datetime-options-property-builder> function updates a CfnDataset$DatetimeOptionsProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :format)]
    (. builder format data))
  (when-some [data (lookup-entry config id :locale-code)]
    (. builder localeCode data))
  (when-some [data (lookup-entry config id :timezone-offset)]
    (. builder timezoneOffset data))
  (.build builder))


(defn cfn-dataset-datetime-options-property-builder
  "Creates a  `CfnDataset$DatetimeOptionsProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-dataset-datetime-options-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-dataset-datetime-options-property-builder> (new CfnDataset$DatetimeOptionsProperty$Builder) id config))


(defn cfn-dataset-excel-options-property-builder>
  "The cfn-dataset-excel-options-property-builder> function updates a CfnDataset$ExcelOptionsProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :header-row)]
    (. builder headerRow data))
  (when-some [data (lookup-entry config id :sheet-indexes)]
    (. builder sheetIndexes data))
  (when-some [data (lookup-entry config id :sheet-names)]
    (. builder sheetNames data))
  (.build builder))


(defn cfn-dataset-excel-options-property-builder
  "Creates a  `CfnDataset$ExcelOptionsProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-dataset-excel-options-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-dataset-excel-options-property-builder> (new CfnDataset$ExcelOptionsProperty$Builder) id config))


(defn cfn-dataset-files-limit-property-builder>
  "The cfn-dataset-files-limit-property-builder> function updates a CfnDataset$FilesLimitProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :max-files)]
    (. builder maxFiles data))
  (when-some [data (lookup-entry config id :order)]
    (. builder order data))
  (when-some [data (lookup-entry config id :ordered-by)]
    (. builder orderedBy data))
  (.build builder))


(defn cfn-dataset-files-limit-property-builder
  "Creates a  `CfnDataset$FilesLimitProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-dataset-files-limit-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-dataset-files-limit-property-builder> (new CfnDataset$FilesLimitProperty$Builder) id config))


(defn cfn-dataset-filter-expression-property-builder>
  "The cfn-dataset-filter-expression-property-builder> function updates a CfnDataset$FilterExpressionProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :values-map)]
    (. builder valuesMap data))
  (.build builder))


(defn cfn-dataset-filter-expression-property-builder
  "Creates a  `CfnDataset$FilterExpressionProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-dataset-filter-expression-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-dataset-filter-expression-property-builder> (new CfnDataset$FilterExpressionProperty$Builder) id config))


(defn cfn-dataset-filter-value-property-builder>
  "The cfn-dataset-filter-value-property-builder> function updates a CfnDataset$FilterValueProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :value)]
    (. builder value data))
  (when-some [data (lookup-entry config id :value-reference)]
    (. builder valueReference data))
  (.build builder))


(defn cfn-dataset-filter-value-property-builder
  "Creates a  `CfnDataset$FilterValueProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-dataset-filter-value-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-dataset-filter-value-property-builder> (new CfnDataset$FilterValueProperty$Builder) id config))


(defn cfn-dataset-format-options-property-builder>
  "The cfn-dataset-format-options-property-builder> function updates a CfnDataset$FormatOptionsProperty$Builder instance using the provided configuration.
...
| Field | DataType | Lookup Function | Data Key |
|---|---|---|---|
| `csv` | software.amazon.awscdk.services.databrew.CfnDataset$CsvOptionsProperty | [[cdk.support/lookup-entry]] | `:csv` |
| `excel` | software.amazon.awscdk.services.databrew.CfnDataset$ExcelOptionsProperty | [[cdk.support/lookup-entry]] | `:excel` |
| `json` | software.amazon.awscdk.services.databrew.CfnDataset$JsonOptionsProperty | [[cdk.support/lookup-entry]] | `:json` |
"
  [^CfnDataset$FormatOptionsProperty$Builder builder id config]
  (when-some [data (lookup-entry config id :csv)]
    (. builder csv data))
  (when-some [data (lookup-entry config id :excel)]
    (. builder excel data))
...
(defn cfn-dataset-format-options-property-builder
  "Creates a  `CfnDataset$FormatOptionsProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-dataset-format-options-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-dataset-format-options-property-builder> (new CfnDataset$FormatOptionsProperty$Builder) id config))


(defn cfn-dataset-input-property-builder>
  "The cfn-dataset-input-property-builder> function updates a CfnDataset$InputProperty$Builder instance using the provided configuration.
...
| Field | DataType | Lookup Function | Data Key |
|---|---|---|---|
| `dataCatalogInputDefinition` | software.amazon.awscdk.services.databrew.CfnDataset$DataCatalogInputDefinitionProperty | [[cdk.support/lookup-entry]] | `:data-catalog-input-definition` |
| `databaseInputDefinition` | software.amazon.awscdk.services.databrew.CfnDataset$DatabaseInputDefinitionProperty | [[cdk.support/lookup-entry]] | `:database-input-definition` |
| `metadata` | software.amazon.awscdk.services.databrew.CfnDataset$MetadataProperty | [[cdk.support/lookup-entry]] | `:metadata` |
| `s3InputDefinition` | software.amazon.awscdk.services.databrew.CfnDataset$S3LocationProperty | [[cdk.support/lookup-entry]] | `:s3-input-definition` |
"
  [^CfnDataset$InputProperty$Builder builder id config]
  (when-some [data (lookup-entry config id :data-catalog-input-definition)]
    (. builder dataCatalogInputDefinition data))
  (when-some [data (lookup-entry config id :database-input-definition)]
    (. builder databaseInputDefinition data))
  (when-some [data (lookup-entry config id :metadata)]
    (. builder metadata data))
  (when-some [data (lookup-entry config id :s3-input-definition)]
    (. builder s3InputDefinition data))
  (.build builder))


(defn cfn-dataset-input-property-builder
  "Creates a  `CfnDataset$InputProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-dataset-input-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-dataset-input-property-builder> (new CfnDataset$InputProperty$Builder) id config))


(defn cfn-dataset-json-options-property-builder>
  "The cfn-dataset-json-options-property-builder> function updates a CfnDataset$JsonOptionsProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :multi-line)]
    (. builder multiLine data))
  (.build builder))


(defn cfn-dataset-json-options-property-builder
  "Creates a  `CfnDataset$JsonOptionsProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-dataset-json-options-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-dataset-json-options-property-builder> (new CfnDataset$JsonOptionsProperty$Builder) id config))


(defn cfn-dataset-metadata-property-builder>
  "The cfn-dataset-metadata-property-builder> function updates a CfnDataset$MetadataProperty$Builder instance using the provided configuration.
...
(defn cfn-dataset-metadata-property-builder
  "Creates a  `CfnDataset$MetadataProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-dataset-metadata-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-dataset-metadata-property-builder> (new CfnDataset$MetadataProperty$Builder) id config))


(defn cfn-dataset-path-options-property-builder>
  "The cfn-dataset-path-options-property-builder> function updates a CfnDataset$PathOptionsProperty$Builder instance using the provided configuration.
...
| Field | DataType | Lookup Function | Data Key |
|---|---|---|---|
| `filesLimit` | software.amazon.awscdk.services.databrew.CfnDataset$FilesLimitProperty | [[cdk.support/lookup-entry]] | `:files-limit` |
| `lastModifiedDateCondition` | software.amazon.awscdk.services.databrew.CfnDataset$FilterExpressionProperty | [[cdk.support/lookup-entry]] | `:last-modified-date-condition` |
| `parameters` | java.util.List | [[cdk.support/lookup-entry]] | `:parameters` |
"
  [^CfnDataset$PathOptionsProperty$Builder builder id config]
  (when-some [data (lookup-entry config id :files-limit)]
    (. builder filesLimit data))
  (when-some [data (lookup-entry config id :last-modified-date-condition)]
...
(defn cfn-dataset-path-options-property-builder
  "Creates a  `CfnDataset$PathOptionsProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-dataset-path-options-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-dataset-path-options-property-builder> (new CfnDataset$PathOptionsProperty$Builder) id config))


(defn cfn-dataset-path-parameter-property-builder>
  "The cfn-dataset-path-parameter-property-builder> function updates a CfnDataset$PathParameterProperty$Builder instance using the provided configuration.
...
| Field | DataType | Lookup Function | Data Key |
|---|---|---|---|
| `datasetParameter` | software.amazon.awscdk.services.databrew.CfnDataset$DatasetParameterProperty | [[cdk.support/lookup-entry]] | `:dataset-parameter` |
...
"
  ...
  (when-some [data (lookup-entry config id :dataset-parameter)]
    (. builder datasetParameter data))
  (when-some [data (lookup-entry config id :path-parameter-name)]
    (. builder pathParameterName data))
  (.build builder))


(defn cfn-dataset-path-parameter-property-builder
  "Creates a  `CfnDataset$PathParameterProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-dataset-path-parameter-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-dataset-path-parameter-property-builder> (new CfnDataset$PathParameterProperty$Builder) id config))
...
(defn cfn-dataset-s3-location-property-builder>
  "The cfn-dataset-s3-location-property-builder> function updates a CfnDataset$S3LocationProperty$Builder instance using the provided configuration.
...
(defn cfn-dataset-s3-location-property-builder
  "Creates a  `CfnDataset$S3LocationProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-dataset-s3-location-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-dataset-s3-location-property-builder> (new CfnDataset$S3LocationProperty$Builder) id config))


(defn cfn-job-allowed-statistics-property-builder>
  "The cfn-job-allowed-statistics-property-builder> function updates a CfnJob$AllowedStatisticsProperty$Builder instance using the provided configuration.
...
(defn cfn-job-allowed-statistics-property-builder
  "Creates a  `CfnJob$AllowedStatisticsProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-allowed-statistics-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-job-allowed-statistics-property-builder> (new CfnJob$AllowedStatisticsProperty$Builder) id config))
...
| Field | DataType | Lookup Function | Data Key |
|---|---|---|---|
| `dataCatalogOutputs` | java.util.List | [[cdk.support/lookup-entry]] | `:data-catalog-outputs` |
...
| `profileConfiguration` | software.amazon.awscdk.services.databrew.CfnJob$ProfileConfigurationProperty | [[cdk.support/lookup-entry]] | `:profile-configuration` |
| `projectName` | java.lang.String | [[cdk.support/lookup-entry]] | `:project-name` |
| `recipe` | software.amazon.awscdk.services.databrew.CfnJob$RecipeProperty | [[cdk.support/lookup-entry]] | `:recipe` |
...
"
  ...
  (when-some [data (lookup-entry config id :data-catalog-outputs)]
    (. builder dataCatalogOutputs data))
  (when-some [data (lookup-entry config id :database-outputs)]
  ...
  (when-some [data (lookup-entry config id :job-sample)]
    (. builder jobSample data))
  (when-some [data (lookup-entry config id :log-subscription)]
  ...
  (when-some [data (lookup-entry config id :type)]
    (. builder type data))
  (when-some [data (lookup-entry config id :validation-configurations)]
    (. builder validationConfigurations data))
  (.build builder))


(defn cfn-job-builder
  "Creates a  `CfnJob$Builder` instance using a scope and ID, applies the data configuration using the [[cfn-job-builder>]] function, then builds it.  Takes the following arguments: 
| Argument | DataType | Description |
|---|---|---|
| `scope` | software.constructs.Construct | The parent scope construct of the object being built. |
| `id` | String or Keyword or Symbol | Value to use as both the ID of the object being build and the namespace when looking up configuration values. |
...
(defn cfn-job-column-selector-property-builder>
  "The cfn-job-column-selector-property-builder> function updates a CfnJob$ColumnSelectorProperty$Builder instance using the provided configuration.
...
(defn cfn-job-column-selector-property-builder
  "Creates a  `CfnJob$ColumnSelectorProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-column-selector-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-job-column-selector-property-builder> (new CfnJob$ColumnSelectorProperty$Builder) id config))


(defn cfn-job-column-statistics-configuration-property-builder>
  "The cfn-job-column-statistics-configuration-property-builder> function updates a CfnJob$ColumnStatisticsConfigurationProperty$Builder instance using the provided configuration.
...
(defn cfn-job-column-statistics-configuration-property-builder
  "Creates a  `CfnJob$ColumnStatisticsConfigurationProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-column-statistics-configuration-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-job-column-statistics-configuration-property-builder> (new CfnJob$ColumnStatisticsConfigurationProperty$Builder) id config))


(defn cfn-job-csv-output-options-property-builder>
  "The cfn-job-csv-output-options-property-builder> function updates a CfnJob$CsvOutputOptionsProperty$Builder instance using the provided configuration.
...
(defn cfn-job-csv-output-options-property-builder
  "Creates a  `CfnJob$CsvOutputOptionsProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-csv-output-options-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-job-csv-output-options-property-builder> (new CfnJob$CsvOutputOptionsProperty$Builder) id config))


(defn cfn-job-data-catalog-output-property-builder>
  "The cfn-job-data-catalog-output-property-builder> function updates a CfnJob$DataCatalogOutputProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :database-name)]
    (. builder databaseName data))
  (when-some [data (lookup-entry config id :database-options)]
    (. builder databaseOptions data))
  (when-some [data (lookup-entry config id :overwrite)]
    (. builder overwrite data))
  (when-some [data (lookup-entry config id :s3-options)]
...
(defn cfn-job-data-catalog-output-property-builder
  "Creates a  `CfnJob$DataCatalogOutputProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-data-catalog-output-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-job-data-catalog-output-property-builder> (new CfnJob$DataCatalogOutputProperty$Builder) id config))


(defn cfn-job-database-output-property-builder>
  "The cfn-job-database-output-property-builder> function updates a CfnJob$DatabaseOutputProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :database-options)]
    (. builder databaseOptions data))
  (when-some [data (lookup-entry config id :database-output-mode)]
...
(defn cfn-job-database-output-property-builder
  "Creates a  `CfnJob$DatabaseOutputProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-database-output-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-job-database-output-property-builder> (new CfnJob$DatabaseOutputProperty$Builder) id config))


(defn cfn-job-database-table-output-options-property-builder>
  "The cfn-job-database-table-output-options-property-builder> function updates a CfnJob$DatabaseTableOutputOptionsProperty$Builder instance using the provided configuration.
...
(defn cfn-job-database-table-output-options-property-builder
  "Creates a  `CfnJob$DatabaseTableOutputOptionsProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-database-table-output-options-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-job-database-table-output-options-property-builder> (new CfnJob$DatabaseTableOutputOptionsProperty$Builder) id config))


(defn cfn-job-entity-detector-configuration-property-builder>
  "The cfn-job-entity-detector-configuration-property-builder> function updates a CfnJob$EntityDetectorConfigurationProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :allowed-statistics)]
...
(defn cfn-job-entity-detector-configuration-property-builder
  "Creates a  `CfnJob$EntityDetectorConfigurationProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-entity-detector-configuration-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-job-entity-detector-configuration-property-builder> (new CfnJob$EntityDetectorConfigurationProperty$Builder) id config))


(defn cfn-job-job-sample-property-builder>
  "The cfn-job-job-sample-property-builder> function updates a CfnJob$JobSampleProperty$Builder instance using the provided configuration.
...
(defn cfn-job-job-sample-property-builder
  "Creates a  `CfnJob$JobSampleProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-job-sample-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-job-job-sample-property-builder> (new CfnJob$JobSampleProperty$Builder) id config))


(defn cfn-job-output-format-options-property-builder>
  "The cfn-job-output-format-options-property-builder> function updates a CfnJob$OutputFormatOptionsProperty$Builder instance using the provided configuration.
...
(defn cfn-job-output-format-options-property-builder
  "Creates a  `CfnJob$OutputFormatOptionsProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-output-format-options-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-job-output-format-options-property-builder> (new CfnJob$OutputFormatOptionsProperty$Builder) id config))


(defn cfn-job-output-location-property-builder>
  "The cfn-job-output-location-property-builder> function updates a CfnJob$OutputLocationProperty$Builder instance using the provided configuration.
...
(defn cfn-job-output-location-property-builder
  "Creates a  `CfnJob$OutputLocationProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-output-location-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-job-output-location-property-builder> (new CfnJob$OutputLocationProperty$Builder) id config))


(defn cfn-job-output-property-builder>
  "The cfn-job-output-property-builder> function updates a CfnJob$OutputProperty$Builder instance using the provided configuration.
...
| `format` | java.lang.String | [[cdk.support/lookup-entry]] | `:format` |
| `formatOptions` | software.amazon.awscdk.services.databrew.CfnJob$OutputFormatOptionsProperty | [[cdk.support/lookup-entry]] | `:format-options` |
...
"
  ...
  (when-some [data (lookup-entry config id :max-output-files)]
    (. builder maxOutputFiles data))
  (when-some [data (lookup-entry config id :overwrite)]
    (. builder overwrite data))
  (when-some [data (lookup-entry config id :partition-columns)]
    (. builder partitionColumns data))
  (.build builder))


(defn cfn-job-output-property-builder
  "Creates a  `CfnJob$OutputProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-output-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-job-output-property-builder> (new CfnJob$OutputProperty$Builder) id config))


(defn cfn-job-profile-configuration-property-builder>
  "The cfn-job-profile-configuration-property-builder> function updates a CfnJob$ProfileConfigurationProperty$Builder instance using the provided configuration.
...
| `entityDetectorConfiguration` | software.amazon.awscdk.services.databrew.CfnJob$EntityDetectorConfigurationProperty | [[cdk.support/lookup-entry]] | `:entity-detector-configuration` |
...
"
  ...
  (when-some [data (lookup-entry config id :column-statistics-configurations)]
    (. builder columnStatisticsConfigurations data))
  (when-some [data (lookup-entry config id :dataset-statistics-configuration)]
    (. builder datasetStatisticsConfiguration data))
  (when-some [data (lookup-entry config id :entity-detector-configuration)]
    (. builder entityDetectorConfiguration data))
  (when-some [data (lookup-entry config id :profile-columns)]
    (. builder profileColumns data))
  (.build builder))


(defn cfn-job-profile-configuration-property-builder
  "Creates a  `CfnJob$ProfileConfigurationProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-profile-configuration-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-job-profile-configuration-property-builder> (new CfnJob$ProfileConfigurationProperty$Builder) id config))
...
(defn cfn-job-props-builder
  "Creates a  `CfnJobProps$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-props-builder>]] function, then builds it.  Takes the following arguments: 
...
(defn cfn-job-recipe-property-builder>
  "The cfn-job-recipe-property-builder> function updates a CfnJob$RecipeProperty$Builder instance using the provided configuration.
...
(defn cfn-job-recipe-property-builder
  "Creates a  `CfnJob$RecipeProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-recipe-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-job-recipe-property-builder> (new CfnJob$RecipeProperty$Builder) id config))


(defn cfn-job-s3-location-property-builder>
  "The cfn-job-s3-location-property-builder> function updates a CfnJob$S3LocationProperty$Builder instance using the provided configuration.
...
(defn cfn-job-s3-location-property-builder
  "Creates a  `CfnJob$S3LocationProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-s3-location-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-job-s3-location-property-builder> (new CfnJob$S3LocationProperty$Builder) id config))


(defn cfn-job-s3-table-output-options-property-builder>
  "The cfn-job-s3-table-output-options-property-builder> function updates a CfnJob$S3TableOutputOptionsProperty$Builder instance using the provided configuration.
...
(defn cfn-job-s3-table-output-options-property-builder
  "Creates a  `CfnJob$S3TableOutputOptionsProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-s3-table-output-options-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-job-s3-table-output-options-property-builder> (new CfnJob$S3TableOutputOptionsProperty$Builder) id config))


(defn cfn-job-statistic-override-property-builder>
  "The cfn-job-statistic-override-property-builder> function updates a CfnJob$StatisticOverrideProperty$Builder instance using the provided configuration.
...
(defn cfn-job-statistic-override-property-builder
  "Creates a  `CfnJob$StatisticOverrideProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-statistic-override-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-job-statistic-override-property-builder> (new CfnJob$StatisticOverrideProperty$Builder) id config))


(defn cfn-job-statistics-configuration-property-builder>
  "The cfn-job-statistics-configuration-property-builder> function updates a CfnJob$StatisticsConfigurationProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :included-statistics)]
...
(defn cfn-job-statistics-configuration-property-builder
  "Creates a  `CfnJob$StatisticsConfigurationProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-statistics-configuration-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-job-statistics-configuration-property-builder> (new CfnJob$StatisticsConfigurationProperty$Builder) id config))


(defn cfn-job-validation-configuration-property-builder>
  "The cfn-job-validation-configuration-property-builder> function updates a CfnJob$ValidationConfigurationProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :ruleset-arn)]
    (. builder rulesetArn data))
  (when-some [data (lookup-entry config id :validation-mode)]
    (. builder validationMode data))
  (.build builder))


(defn cfn-job-validation-configuration-property-builder
  "Creates a  `CfnJob$ValidationConfigurationProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-job-validation-configuration-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-job-validation-configuration-property-builder> (new CfnJob$ValidationConfigurationProperty$Builder) id config))
...
"
  ...
  (when-some [data (lookup-entry config id :name)]
    (. builder name data))
  (when-some [data (lookup-entry config id :recipe-name)]
...
(defn cfn-project-sample-property-builder>
  "The cfn-project-sample-property-builder> function updates a CfnProject$SampleProperty$Builder instance using the provided configuration.
...
(defn cfn-project-sample-property-builder
  "Creates a  `CfnProject$SampleProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-project-sample-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-project-sample-property-builder> (new CfnProject$SampleProperty$Builder) id config))


(defn cfn-recipe-action-property-builder>
  "The cfn-recipe-action-property-builder> function updates a CfnRecipe$ActionProperty$Builder instance using the provided configuration.
...
(defn cfn-recipe-action-property-builder
  "Creates a  `CfnRecipe$ActionProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-recipe-action-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-recipe-action-property-builder> (new CfnRecipe$ActionProperty$Builder) id config))


(defn cfn-recipe-builder>
  "The cfn-recipe-builder> function updates a CfnRecipe$Builder instance using the provided configuration.
...
(defn cfn-recipe-builder
  "Creates a  `CfnRecipe$Builder` instance using a scope and ID, applies the data configuration using the [[cfn-recipe-builder>]] function, then builds it.  Takes the following arguments: 
| Argument | DataType | Description |
|---|---|---|
| `scope` | software.constructs.Construct | The parent scope construct of the object being built. |
| `id` | String or Keyword or Symbol | Value to use as both the ID of the object being build and the namespace when looking up configuration values. |
| `config` | map | Data configuration |"
  [^software.constructs.Construct scope id config]
  (cfn-recipe-builder> (CfnRecipe$Builder/create scope (name id)) id config))


(defn cfn-recipe-condition-expression-property-builder>
  "The cfn-recipe-condition-expression-property-builder> function updates a CfnRecipe$ConditionExpressionProperty$Builder instance using the provided configuration.
...
"
  ...
  (when-some [data (lookup-entry config id :target-column)]
...
(defn cfn-recipe-condition-expression-property-builder
  "Creates a  `CfnRecipe$ConditionExpressionProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-recipe-condition-expression-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-recipe-condition-expression-property-builder> (new CfnRecipe$ConditionExpressionProperty$Builder) id config))


(defn cfn-recipe-data-catalog-input-definition-property-builder>
  "The cfn-recipe-data-catalog-input-definition-property-builder> function updates a CfnRecipe$DataCatalogInputDefinitionProperty$Builder instance using the provided configuration.
...
(defn cfn-recipe-data-catalog-input-definition-property-builder
  "Creates a  `CfnRecipe$DataCatalogInputDefinitionProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-recipe-data-catalog-input-definition-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-recipe-data-catalog-input-definition-property-builder> (new CfnRecipe$DataCatalogInputDefinitionProperty$Builder) id config))


(defn cfn-recipe-input-property-builder>
  "The cfn-recipe-input-property-builder> function updates a CfnRecipe$InputProperty$Builder instance using the provided configuration.
...
| `s3InputDefinition` | software.amazon.awscdk.services.databrew.CfnRecipe$S3LocationProperty | [[cdk.support/lookup-entry]] | `:s3-input-definition` |
"
...
(defn cfn-recipe-input-property-builder
  "Creates a  `CfnRecipe$InputProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-recipe-input-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-recipe-input-property-builder> (new CfnRecipe$InputProperty$Builder) id config))


(defn cfn-recipe-props-builder>
  "The cfn-recipe-props-builder> function updates a CfnRecipeProps$Builder instance using the provided configuration.
...
(defn cfn-recipe-props-builder
  "Creates a  `CfnRecipeProps$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-recipe-props-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-recipe-props-builder> (new CfnRecipeProps$Builder) id config))


(defn cfn-recipe-recipe-parameters-property-builder>
  "The cfn-recipe-recipe-parameters-property-builder> function updates a CfnRecipe$RecipeParametersProperty$Builder instance using the provided configuration.
...
| `secondaryInputs` | java.util.List | [[cdk.support/lookup-entry]] | `:secondary-inputs` |
| `sheetIndexes` | java.util.List | [[cdk.support/lookup-entry]] | `:sheet-indexes` |
| `sheetNames` | java.util.List | [[cdk.support/lookup-entry]] | `:sheet-names` |
...
"
  ...
  (when-some [data (lookup-entry config id :aggregate-function)]
    (. builder aggregateFunction data))
  (when-some [data (lookup-entry config id :base)]
    (. builder base data))
  (when-some [data (lookup-entry config id :case-statement)]
    (. builder caseStatement data))
  (when-some [data (lookup-entry config id :category-map)]
    (. builder categoryMap data))
  (when-some [data (lookup-entry config id :chars-to-remove)]
    (. builder charsToRemove data))
  (when-some [data (lookup-entry config id :collapse-consecutive-whitespace)]
    (. builder collapseConsecutiveWhitespace data))
  (when-some [data (lookup-entry config id :column-data-type)]
    (. builder columnDataType data))
  (when-some [data (lookup-entry config id :column-range)]
    (. builder columnRange data))
  (when-some [data (lookup-entry config id :count)]
    (. builder count data))
  (when-some [data (lookup-entry config id :custom-characters)]
    (. builder customCharacters data))
  (when-some [data (lookup-entry config id :custom-stop-words)]
    (. builder customStopWords data))
  (when-some [data (lookup-entry config id :custom-value)]
    (. builder customValue data))
  (when-some [data (lookup-entry config id :datasets-columns)]
    (. builder datasetsColumns data))
  (when-some [data (lookup-entry config id :date-add-value)]
    (. builder dateAddValue data))
  (when-some [data (lookup-entry config id :date-time-format)]
    (. builder dateTimeFormat data))
  (when-some [data (lookup-entry config id :date-time-parameters)]
    (. builder dateTimeParameters data))
  (when-some [data (lookup-entry config id :delete-other-rows)]
    (. builder deleteOtherRows data))
  (when-some [data (lookup-entry config id :delimiter)]
    (. builder delimiter data))
  (when-some [data (lookup-entry config id :end-pattern)]
    (. builder endPattern data))
  (when-some [data (lookup-entry config id :end-position)]
    (. builder endPosition data))
  (when-some [data (lookup-entry config id :end-value)]
    (. builder endValue data))
  (when-some [data (lookup-entry config id :expand-contractions)]
    (. builder expandContractions data))
  (when-some [data (lookup-entry config id :exponent)]
    (. builder exponent data))
  (when-some [data (lookup-entry config id :false-string)]
    (. builder falseString data))
  (when-some [data (lookup-entry config id :group-by-agg-function-options)]
    (. builder groupByAggFunctionOptions data))
  (when-some [data (lookup-entry config id :group-by-columns)]
    (. builder groupByColumns data))
  (when-some [data (lookup-entry config id :hidden-columns)]
    (. builder hiddenColumns data))
  (when-some [data (lookup-entry config id :ignore-case)]
    (. builder ignoreCase data))
  (when-some [data (lookup-entry config id :include-in-split)]
    (. builder includeInSplit data))
  (when-some [data (lookup-entry config id :input)]
    (. builder input data))
  (when-some [data (lookup-entry config id :interval)]
    (. builder interval data))
  (when-some [data (lookup-entry config id :is-text)]
    (. builder isText data))
  (when-some [data (lookup-entry config id :join-keys)]
    (. builder joinKeys data))
  (when-some [data (lookup-entry config id :join-type)]
    (. builder joinType data))
  (when-some [data (lookup-entry config id :left-columns)]
  ...
  (when-some [data (lookup-entry config id :map-type)]
    (. builder mapType data))
  (when-some [data (lookup-entry config id :mode-type)]
    (. builder modeType data))
  (when-some [data (lookup-entry config id :multi-line)]
    (. builder multiLine data))
  (when-some [data (lookup-entry config id :num-rows)]
    (. builder numRows data))
  (when-some [data (lookup-entry config id :num-rows-after)]
    (. builder numRowsAfter data))
  (when-some [data (lookup-entry config id :num-rows-before)]
    (. builder numRowsBefore data))
  (when-some [data (lookup-entry config id :order-by-column)]
    (. builder orderByColumn data))
  (when-some [data (lookup-entry config id :order-by-columns)]
    (. builder orderByColumns data))
  (when-some [data (lookup-entry config id :other)]
    (. builder other data))
  (when-some [data (lookup-entry config id :pattern)]
    (. builder pattern data))
  (when-some [data (lookup-entry config id :pattern-option1)]
    (. builder patternOption1 data))
  (when-some [data (lookup-entry config id :pattern-option2)]
    (. builder patternOption2 data))
  (when-some [data (lookup-entry config id :pattern-options)]
  ...
  (when-some [data (lookup-entry config id :remove-all-punctuation)]
    (. builder removeAllPunctuation data))
  (when-some [data (lookup-entry config id :remove-all-quotes)]
    (. builder removeAllQuotes data))
  (when-some [data (lookup-entry config id :remove-all-whitespace)]
    (. builder removeAllWhitespace data))
  (when-some [data (lookup-entry config id :remove-custom-characters)]
    (. builder removeCustomCharacters data))
  (when-some [data (lookup-entry config id :remove-custom-value)]
    (. builder removeCustomValue data))
  (when-some [data (lookup-entry config id :remove-leading-and-trailing-punctuation)]
    (. builder removeLeadingAndTrailingPunctuation data))
  (when-some [data (lookup-entry config id :remove-leading-and-trailing-quotes)]
    (. builder removeLeadingAndTrailingQuotes data))
  (when-some [data (lookup-entry config id :remove-leading-and-trailing-whitespace)]
    (. builder removeLeadingAndTrailingWhitespace data))
  (when-some [data (lookup-entry config id :remove-letters)]
    (. builder removeLetters data))
  (when-some [data (lookup-entry config id :remove-numbers)]
    (. builder removeNumbers data))
  (when-some [data (lookup-entry config id :remove-source-column)]
    (. builder removeSourceColumn data))
  (when-some [data (lookup-entry config id :remove-special-characters)]
    (. builder removeSpecialCharacters data))
  (when-some [data (lookup-entry config id :right-columns)]
    (. builder rightColumns data))
  (when-some [data (lookup-entry config id :sample-size)]
    (. builder sampleSize data))
  (when-some [data (lookup-entry config id :sample-type)]
    (. builder sampleType data))
  (when-some [data (lookup-entry config id :second-input)]
    (. builder secondInput data))
  (when-some [data (lookup-entry config id :secondary-inputs)]
  ...
  (when-some [data (lookup-entry config id :source-column1)]
    (. builder sourceColumn1 data))
  (when-some [data (lookup-entry config id :source-column2)]
    (. builder sourceColumn2 data))
  (when-some [data (lookup-entry config id :source-columns)]
    (. builder sourceColumns data))
  (when-some [data (lookup-entry config id :start-column-index)]
    (. builder startColumnIndex data))
  (when-some [data (lookup-entry config id :start-pattern)]
    (. builder startPattern data))
  (when-some [data (lookup-entry config id :start-position)]
    (. builder startPosition data))
  (when-some [data (lookup-entry config id :start-value)]
    (. builder startValue data))
  (when-some [data (lookup-entry config id :stemming-mode)]
    (. builder stemmingMode data))
  (when-some [data (lookup-entry config id :step-count)]
    (. builder stepCount data))
  (when-some [data (lookup-entry config id :step-index)]
    (. builder stepIndex data))
  (when-some [data (lookup-entry config id :stop-words-mode)]
    (. builder stopWordsMode data))
  (when-some [data (lookup-entry config id :strategy)]
    (. builder strategy data))
  (when-some [data (lookup-entry config id :target-column)]
    (. builder targetColumn data))
  (when-some [data (lookup-entry config id :target-column-names)]
    (. builder targetColumnNames data))
  (when-some [data (lookup-entry config id :target-date-format)]
    (. builder targetDateFormat data))
  (when-some [data (lookup-entry config id :target-index)]
    (. builder targetIndex data))
  (when-some [data (lookup-entry config id :time-zone)]
    (. builder timeZone data))
  (when-some [data (lookup-entry config id :tokenizer-pattern)]
    (. builder tokenizerPattern data))
  (when-some [data (lookup-entry config id :true-string)]
    (. builder trueString data))
  (when-some [data (lookup-entry config id :udf-lang)]
    (. builder udfLang data))
  (when-some [data (lookup-entry config id :units)]
    (. builder units data))
  (when-some [data (lookup-entry config id :unpivot-column)]
    (. builder unpivotColumn data))
  (when-some [data (lookup-entry config id :upper-bound)]
    (. builder upperBound data))
  (when-some [data (lookup-entry config id :use-new-data-frame)]
    (. builder useNewDataFrame data))
  (when-some [data (lookup-entry config id :value)]
    (. builder value data))
  (when-some [data (lookup-entry config id :value1)]
    (. builder value1 data))
  (when-some [data (lookup-entry config id :value2)]
    (. builder value2 data))
  (when-some [data (lookup-entry config id :value-column)]
    (. builder valueColumn data))
  (when-some [data (lookup-entry config id :view-frame)]
    (. builder viewFrame data))
  (.build builder))


(defn cfn-recipe-recipe-parameters-property-builder
  "Creates a  `CfnRecipe$RecipeParametersProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-recipe-recipe-parameters-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-recipe-recipe-parameters-property-builder> (new CfnRecipe$RecipeParametersProperty$Builder) id config))


(defn cfn-recipe-recipe-step-property-builder>
  "The cfn-recipe-recipe-step-property-builder> function updates a CfnRecipe$RecipeStepProperty$Builder instance using the provided configuration.
...
| Field | DataType | Lookup Function | Data Key |
|---|---|---|---|
| `action` | software.amazon.awscdk.services.databrew.CfnRecipe$ActionProperty | [[cdk.support/lookup-entry]] | `:action` |
...
"
  ...
  (when-some [data (lookup-entry config id :action)]
    (. builder action data))
  (when-some [data (lookup-entry config id :condition-expressions)]
    (. builder conditionExpressions data))
  (.build builder))


(defn cfn-recipe-recipe-step-property-builder
  "Creates a  `CfnRecipe$RecipeStepProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-recipe-recipe-step-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-recipe-recipe-step-property-builder> (new CfnRecipe$RecipeStepProperty$Builder) id config))


(defn cfn-recipe-s3-location-property-builder>
  "The cfn-recipe-s3-location-property-builder> function updates a CfnRecipe$S3LocationProperty$Builder instance using the provided configuration.
...
(defn cfn-recipe-s3-location-property-builder
  "Creates a  `CfnRecipe$S3LocationProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-recipe-s3-location-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-recipe-s3-location-property-builder> (new CfnRecipe$S3LocationProperty$Builder) id config))


(defn cfn-recipe-secondary-input-property-builder>
  "The cfn-recipe-secondary-input-property-builder> function updates a CfnRecipe$SecondaryInputProperty$Builder instance using the provided configuration.
...
(defn cfn-recipe-secondary-input-property-builder
  "Creates a  `CfnRecipe$SecondaryInputProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-recipe-secondary-input-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-recipe-secondary-input-property-builder> (new CfnRecipe$SecondaryInputProperty$Builder) id config))


(defn cfn-ruleset-builder>
  "The cfn-ruleset-builder> function updates a CfnRuleset$Builder instance using the provided configuration.
...
(defn cfn-ruleset-builder
  "Creates a  `CfnRuleset$Builder` instance using a scope and ID, applies the data configuration using the [[cfn-ruleset-builder>]] function, then builds it.  Takes the following arguments: 
| Argument | DataType | Description |
|---|---|---|
| `scope` | software.constructs.Construct | The parent scope construct of the object being built. |
| `id` | String or Keyword or Symbol | Value to use as both the ID of the object being build and the namespace when looking up configuration values. |
| `config` | map | Data configuration |"
  [^software.constructs.Construct scope id config]
  (cfn-ruleset-builder> (CfnRuleset$Builder/create scope (name id)) id config))


(defn cfn-ruleset-column-selector-property-builder>
  "The cfn-ruleset-column-selector-property-builder> function updates a CfnRuleset$ColumnSelectorProperty$Builder instance using the provided configuration.
...
(defn cfn-ruleset-column-selector-property-builder
  "Creates a  `CfnRuleset$ColumnSelectorProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-ruleset-column-selector-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-ruleset-column-selector-property-builder> (new CfnRuleset$ColumnSelectorProperty$Builder) id config))


(defn cfn-ruleset-props-builder>
  "The cfn-ruleset-props-builder> function updates a CfnRulesetProps$Builder instance using the provided configuration.
...
(defn cfn-ruleset-props-builder
  "Creates a  `CfnRulesetProps$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-ruleset-props-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-ruleset-props-builder> (new CfnRulesetProps$Builder) id config))


(defn cfn-ruleset-rule-property-builder>
  "The cfn-ruleset-rule-property-builder> function updates a CfnRuleset$RuleProperty$Builder instance using the provided configuration.
...
| `columnSelectors` | java.util.List | [[cdk.support/lookup-entry]] | `:column-selectors` |
| `disabled` | java.lang.Boolean | [[cdk.support/lookup-entry]] | `:disabled` |
| `name` | java.lang.String | [[cdk.support/lookup-entry]] | `:name` |
| `substitutionMap` | java.util.List | [[cdk.support/lookup-entry]] | `:substitution-map` |
...
"
  ...
  (when-some [data (lookup-entry config id :check-expression)]
    (. builder checkExpression data))
  (when-some [data (lookup-entry config id :column-selectors)]
    (. builder columnSelectors data))
  (when-some [data (lookup-entry config id :disabled)]
    (. builder disabled data))
  (when-some [data (lookup-entry config id :name)]
    (. builder name data))
  (when-some [data (lookup-entry config id :substitution-map)]
...
(defn cfn-ruleset-rule-property-builder
  "Creates a  `CfnRuleset$RuleProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-ruleset-rule-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-ruleset-rule-property-builder> (new CfnRuleset$RuleProperty$Builder) id config))


(defn cfn-ruleset-substitution-value-property-builder>
  "The cfn-ruleset-substitution-value-property-builder> function updates a CfnRuleset$SubstitutionValueProperty$Builder instance using the provided configuration.
...
(defn cfn-ruleset-substitution-value-property-builder
  "Creates a  `CfnRuleset$SubstitutionValueProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-ruleset-substitution-value-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-ruleset-substitution-value-property-builder> (new CfnRuleset$SubstitutionValueProperty$Builder) id config))


(defn cfn-ruleset-threshold-property-builder>
  "The cfn-ruleset-threshold-property-builder> function updates a CfnRuleset$ThresholdProperty$Builder instance using the provided configuration.
...
(defn cfn-ruleset-threshold-property-builder
  "Creates a  `CfnRuleset$ThresholdProperty$Builder` instance using a no-argument constructor, applies the data configuration using the [[cfn-ruleset-threshold-property-builder>]] function, then builds it.  Takes the following arguments: 
...
| `config` | map | Data configuration |"
  [id config]
  (cfn-ruleset-threshold-property-builder> (new CfnRuleset$ThresholdProperty$Builder) id config))
...
"
  ...
  (when-some [data (lookup-entry config id :job-names)]
