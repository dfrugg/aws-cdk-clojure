...
The `aws-cdk-clojure` library enables the creation of AWS CDK projects in a Clojure-idiomatic way by providing functions which interpret Clojure data into the standard CDK builders and values that the AWS CDK library provides.  The functions handle all the Java-interop to the AWS library so the implementer does not have to.  The functions that are provided typically have two flavors - (1) interpret data into a Builder class and (2) interpret the data into a value such as an enum.
